...
@property (nonatomic, readonly, copy) NSSet<NSString *> * _Nonnull backgroundModes;
@end
...
/// \param dictionary A dictionary representation of the location.
///
...
/// Identifies the feedback as the location of a poor instruction or route choice. This could be used to indicate an ambiguous or poorly-timed turn announcement, or a set of confusing turns.
  MBFeedbackTypeRoutingError = 5,
};




/// <code>NavigationViewController</code> is the base location manager which handles
...
@interface ReplayLocationManager : MBNavigationLocationManager
...
- (nonnull instancetype)initWithLocations:(NSArray<CLLocation *> * _Nonnull)locations OBJC_DESIGNATED_INITIALIZER;
...
/// If true, every 2 minutes the <code>RouteController</code> will check for a faster route for the user.
@property (nonatomic) BOOL checkForFasterRouteInBackground;
...
- (void)updateFeedbackWithFeedbackId:(NSString * _Nonnull)feedbackId type:(enum MBFeedbackType)type description:(NSString * _Nullable)description;
...
/// <code>AlertLevel</code> for the current step.
...
///
/// \param alertLevel Optional <code>AlertLevel</code> to start the <code>RouteProgress</code> at.
///
- (nonnull instancetype)initWithLeg:(MBRouteLeg * _Nonnull)leg stepIndex:(NSInteger)stepIndex alertLevel:(enum MBAlertLevel)alertLevel OBJC_DESIGNATED_INITIALIZER;
...
/// \param alertLevel Optional <code>AlertLevel</code> to start the <code>RouteProgress</code> at.
///
- (nonnull instancetype)initWithRoute:(MBRoute * _Nonnull)route legIndex:(NSInteger)legIndex alertLevel:(enum MBAlertLevel)alertLevel OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end


@interface MBRouteStep (SWIFT_EXTENSION(MapboxCoreNavigation))
/// Returns a string describing the step’s road by its name, route number, or both, depending on the kind of road.
/// \param markedUpWithSSML True to wrap the name and route number in SSML tags that cause them to be read as addresses.
///
///
/// returns:
/// A string describing the step’s road, or <code>nil</code> if the step lacks the information needed to describe the step.
- (NSString * _Nullable)roadDescriptionWithMarkedUpWithSSML:(BOOL)markedUpWithSSML SWIFT_WARN_UNUSED_RESULT;
@end
...
SWIFT_CLASS_NAMED("SpokenDistanceFormatter")
@interface MBSpokenDistanceFormatter : MBDistanceFormatter
/// Returns a more human readable <code>String</code> from a given <code>CLLocationDistance</code>.
/// The user’s <code>Locale</code> is used here to set the units.
- (NSString * _Nonnull)stringFrom:(CLLocationDistance)distance SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)initWithApproximate:(BOOL)approximate OBJC_DESIGNATED_INITIALIZER;
...
SWIFT_CLASS_NAMED("SpokenInstructionFormatter")
@interface MBSpokenInstructionFormatter : NSObject
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
/// Creates a string used for announcing a step.
/// If <code>markUpWithSSML</code> is true, the string will contain <a href="https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/speech-synthesis-markup-language-ssml-reference">SSML</a>. Your speech synthesizer should accept this type of string. See <code>PolleyVoiceController</code>.
- (NSString * _Nonnull)stringWithRouteProgress:(MBRouteProgress * _Nonnull)routeProgress userDistance:(CLLocationDistance)userDistance markUpWithSSML:(BOOL)markUpWithSSML SWIFT_WARN_UNUSED_RESULT;
@end
...
@interface UIWindow (SWIFT_EXTENSION(MapboxCoreNavigation))
